...
const playSound = require('../../assets/sounds/fly.mp3')
const endSound = require('../../assets/sounds/end.mp3')
const playSnd = new Audio(playSound);
const endSnd = new Audio(endSound);
playSnd.volume = 0.15;
playSnd.loop = true;
endSnd.volume = 0.5;
...
export default function WebGLStarter() {
	const { GameState, currentNum, time, unityState, myUnityContext,setCurrentTarget, sound } = React.useContext(Context)
			...
			<div className="crash-text-container">
				{GameState === "BET" ? (
					...
					<div className={`crashtext ${GameState === "GAMEEND" && "red"}`}>
						...
						<div>
							{target >= 1 ? Number(target).toFixed(2) : "1.00"} <span className="font-[900]">x</span>
						</div>
					</div>
				)}
			</div>
