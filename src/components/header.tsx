...
export default function Header() {
  const { state, sound, setSound, scrollRef } = React.useContext(Context)
    ...
    <div className="header flex-none items-center">
      <div className="header-container">
        ...
        <div className="second-block">
          ...
          <button className="howto toggleSnd" onClick={() => setSound(!sound)}>
            <div className={"help-logo "+(sound?"snd-on":"snd-off")}></div>
            <div className="help-msg">{sound?"Sound On":"Sound Off"}</div>
          </button>
          <button className="howto toggleSnd" onClick={() => {scrollRef.current.scrollTop = scrollRef.current.scrollHeight}}>
            <div className={"help-logo history"}></div>
            <div className="help-msg">Bet History</div>
          </button>
          <div className="d-flex">
            <div className="balance">
              <span className="amount">{Number(state.userInfo.balance).toFixed(2)} </span>
              <span className="currency">&nbsp;{ state.userInfo.currency }</span>
            </div>
          </div>
        </div>
      </div>
