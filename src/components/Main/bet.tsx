...
const Bet = ({ index, add, setAdd }: BetProps) => {
	...
	const [cashOut, setCashOut] = React.useState<string>("2.00");
	...
	const minus = (type: FieldNameType) => {
		...
		if (type === "betAmount") {
			...
			if (value[`${index + type}`] - 1 < 0) {
				value[`${index + type}`] = 0
			} else {
				value[`${index + type}`] = Number((Number(value[`${index + type}`]) - 1).toFixed(2))
			}
		}
		update(value);
	}
	...
	const onChangeBlur = (e: number, type: 'cashOutAt' | 'decrease' | 'increase' | 'singleAmount') => {
		...
		if (type === "cashOutAt") {
			if (e < 1.01) {
				...
				setCashOut((Math.round(e * 100) / 100).toFixed(2));
			}
		} else {
			if (e < 0.1) {
	...
	// 	if (betted) {
	// 		if (autoCashoutState) {
	// 			if (Number(cashOut) <= currentTarget) {
	// 				updateUserBetState({ [`${index}betted`]: false });
	// 				callCashOut(Number(cashOut), index);
	// 			}
	// 		}
	// 	}
	// }, [currentTarget, fbetted, sbetted, state.fautoCashoutState, state.sautoCashoutState, state.fautoCashoutX, state.sautoCashoutState])
								...
								<div className="input">
									{betState || betted ?
										...
										<input type="number" value={Number(myBetAmount)}
											onChange={e => {
												Number(e.target.value) > maxBet ? update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { betAmount: maxBet } } }) : Number(e.target.value) < minBet ? update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { betAmount: minBet } } }) :
							...
							<div className="cashout-block">
								<div className="cashout-switcher">
									...
									{betted || betState ? (
										...
										<div onClick={() => { update({ ...state, [`${index}autoCashoutState`]: !autoCashoutState }) }} className={`input-switch ${autoCashoutState ? "" : "off"}`}>
											<span className="oval"></span>
										</div>
									)}
								</div>
										...
										<div className={`snipper small ${autoCashoutState && !betState ? "" : "disabled"}`}>
											<div className="input">
												{autoCashoutState && !betState ? (
													<input type="number"
														onChange={(e) => { update({ ...state, [`${index}autoCashoutX`]: Number(e.target.value) }); setCashOut(e.target.value) }}
														value={cashOut}
														onBlur={(e) => onChangeBlur(Number(e.target.value) || 0, "cashOutAt")}
													/>
												) : (
													<input type="number" value={cashOut} readOnly />
												)}
											</div>
