...
interface ContextType extends GameBetLimit, UserStatusType, GameStatusType {
	...
	sound: boolean
	setSound(attrs:Partial<boolean>)
	scrollRef: any
...
const unityContext = new UnityContext({
	loaderUrl: "/crash/unity/AirCrash.loader.js",
	dataUrl: "/crash/unity/AirCrash.data.unityweb",
	frameworkUrl: "/crash/unity/AirCrash.framework.js.unityweb",
	codeUrl: "/crash/unity/AirCrash.wasm.unityweb"
});
...
const socketURL = process.env.REACT_APP_SERVER_SOCKET_URL as string;
// const socket = io(socketURL);
const socket = io(socketURL, {path: '/crash/socket.io', transports: ['websocket']});
...
export const Provider = ({ children }: any) => {
	...
	React.useEffect(() => {
		...
		// socket.on("myBetState", (data:{user:UserType, type:string}) => {
		// 	setUserBetState({...userBetState, [`${data.type}betState`]: false});
		// })
		...
		if (gameState.GameState === "BET") {
			if (betStatus.fbetState) {
				...
				let data = {
					...
					target: state.fautoCashoutState?state.fautoCashoutX:0,
					type: 'f',
			...
			if (betStatus.sbetState) {
				...
				let data = {
					...
					target: state.sautoCashoutState?state.sautoCashoutX:0,
					type: 's',
	...
	const getMyBets = async () => {
		try {
			let response = await axios.post(`${process.env.REACT_APP_SERVER_API_URL}/my-info`, { id: state.userInfo.userId });
